...
angular.module('starter.controllers', ["ngAnimate", 'commonService', "ui.bootstrap"])
    ...
    .controller('LeftPanelController', ["$scope", "$timeout", "$http", "$filter", "commonService", "$compile", "$sce", function ($scope, $timeout, $http, $filter, commonService, $compile, $sce) {
                ...
                if (menusList.isDefaultLevel) {
                    ...
                    if (selectedItems.filter(function (e) { return e.DisplayName == "CATEGORY/ITEM/BRAND" }).length > 0 && parentList[0].DisplayName != "ADDITIONAL FILTERS") {
                        ...
                        if (menusList.DisplayName.toUpperCase() == "ROW NESTING") {
                            ...
                            tempData = tempData.filter(value => ((rowList[0].Data.filter(e => checkAnyItemPresent(value.FilterID + "", e.FilterID + "", ',', ',')).length > 0) ||
                                 value.DisplayName.toUpperCase() == "SELECT ALL " + appendText + "ITEMS" || value.DisplayName.toUpperCase() == "SELECT ALL ITEMS" || value.DisplayName.toUpperCase() == "CUSTOM GROUP"))
                        }
                        if (menusList.DisplayName.toUpperCase() != "COLUMN" && catList.indexOf(selectedColumn) > -1 && columnList.length > 0) {
                            keepList = $scope.filterList(columnList, keepList);
                    ...
                    if (selectedItems.filter(function (e) { return e.DisplayName == "CATEGORY/ITEM/BRAND" }).length > 0 && parentList[0].DisplayName != "ADDITIONAL FILTERS") {
                        ...
                        if (menusList.DisplayName.toUpperCase() == "ROW NESTING") {
                            ...
                            for (let item in tempData) {
                                ...
                                else if ((rowList[0].Data.filter(e => checkAnyItemPresent(tempData[item].FilterID + "", e.FilterID + "", ',', ',')).length > 0)) {
                                    tempData[item].data = updateChildDetails(tempData[item].data, rowList[0].Data.filter(e => checkAnyItemPresent(tempData[item].FilterID + "", e.FilterID + "", ',', ','))[0].Data)
                                    currTempData.push(tempData[item]);
                                }
                            }
        ...
        function filterChannels(list, menusList) {
            ...
            let retailerNetLastLevel = false;
            ...
            if (channelsList.length > 0) {
                ...
                if (retailerList[0].DisplayName.toUpperCase().trim() == "RETAILER NETS" && retailerList[0].Data.filter(function (e) { return e.DisplayName != "Custom Group Channel" && e.Data.length > 0 }).length > 0)//Last Level in Retailer Net is selected
                    retailerNetLastLevel = true;
                isRetailSelected = 1;
            }
            ...
            if (purchaseBucketData.length > 0) {
                angular.forEach(keepList, function (a, b) {
                    ...
                    else if (isRetailSelected && retailerList.length > 0 && retailerList[0].DisplayName.toUpperCase().trim() == "RETAILER NETS" && !retailerNetLastLevel) {
                        let retailerSelected = [];
                        let retailerBucket = retailerList[0].DisplayName;
                        retailerSelected = $scope.filterList(purchaseList[0].Data.filter(function (e) { return e.DisplayName == retailerBucket }), retailerSelected);
                        let retailerBucketData = purchaseBucketData.filter(function (e) { return e.DisplayName == retailerBucket })[0].data;
                        purchaseBucketData.filter(function (e) { return e.DisplayName == retailerBucket })[0].data = retailerBucketData.filter(function (e) { return (retailerSelected.indexOf(e.DisplayName) > -1 && !e.IsLastLevel) || (e.DisplayName.toUpperCase().indexOf("SELECT ALL") > -1 && e.DisplayName.toUpperCase().indexOf("RETAILERS") > -1) });
                        retailerBucketData = purchaseBucketData.filter(function (e) { return e.DisplayName == retailerBucket })[0].data;
                        retailerBucketData.filter(function (e) { return e.DisplayName.toUpperCase().indexOf("SELECT ALL") == -1 }).filter(function (e) { return e.IsSelectable = false });
                    }
        ...
        //    return result;
        //}
        let checkAnyItemPresent = function (first, second, firstSep, secondSep) {
            let firstList = first.split(firstSep);
            let secondList = second.split(secondSep);
            return firstList.filter((f) =>secondList.findIndex(s=>s == f) > -1).length > 0 ? true : false;
        }
        let updateChildDetails = function (first, second, compare) {
            ...
            for (let item in first) {
                ...
                if (compare == undefined && second.filter(e => checkAnyItemPresent(first[item].FilterID + "", e.FilterID + "", ',', ',')).length > 0) {
                    indexChecked.push(item);
                    //first[item].data = updateChildDetails(first[item].data, second.filter(e => e.FilterID == first[item].FilterID)[0].Data);
                    first[item].data = updateChildDetails(first[item].data, second.filter(e => checkAnyItemPresent(first[item].FilterID + "", e.FilterID + "", ',', ','))[0].Data);
                    ...
                    if (first[item].IsCollapsible == true) {
                        for (let childItem in first) {
                            ...
                            if (checkAnyItemPresent(first[childItem].ParentId + "", first[item].FilterID + "", ',', ',')) {
        ...
        $scope.left_panel_submit = function (type) {
            ...
            let excelFooterType = updateExcelFooterType($scope);
        ...
        function updateExcelFooterType(scope) {
            var flag = 0;
            let column="Occasion %";
            let row="occasion %";
            let rowNesting = "occasion %";
            let columnSelected = selectedItems.filter(function (e) { return e.DisplayName == 'COLUMN' });
            let rowSelected = selectedItems.filter(function (e) { return e.DisplayName == 'ROW' });
            let rowNestingSelected = selectedItems.filter(function (e) { return e.DisplayName == 'ROW NESTING' });
            if (selectedModuleId == "3") {
                ...
                for (var i = 0; i < list.length; ++i) {
                    ...
                    if (pData.indexOf("true") > -1) {
                        //isItemLevel = true;
                        //break;
                        if (columnSelected.length > 0 && columnSelected[0]["Data"].length > 0 && columnSelected[0]["Data"][0].DisplayName == list[i]) {
                            column = "item %";
                        }
                        else if (rowSelected.length > 0 && rowSelected[0]["Data"].length > 0 && rowSelected[0]["Data"][0].DisplayName == list[i]) {
                            row = "item %";
                        }
                        else if (rowNestingSelected.length > 0 && rowNestingSelected[0]["Data"].length > 0 && rowNestingSelected[0]["Data"][0].DisplayName == list[i]) {
                            rowNesting = "item %";
                        }
                    }
